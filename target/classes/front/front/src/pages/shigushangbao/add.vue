<template>
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item" label="事故时间" prop="shigushijian">
				<el-date-picker
					:disabled="true  ||ro.shigushijian"
			...
			<el-form-item class="add-item" label="事故地点" prop="shigudidian">
				<el-input v-model="ruleForm.shigudidian" 
					placeholder="事故地点" clearable :disabled=" false  ||ro.shigudidian"></el-input>
			</el-form-item>
			<el-form-item class="add-item"  label="事故类型" prop="shiguleixing">
				<el-select v-model="ruleForm.shiguleixing" placeholder="请选择事故类型" :disabled=" false  ||ro.shiguleixing" >
					<el-option
			...
			<el-form-item class="add-item" label="现场照片" v-if="type!='cross' || (type=='cross' && !ro.xianchangzhaopian)" prop="xianchangzhaopian">
				<file-upload
					tip="点击上传现场照片"
					action="file/upload"
					:limit="3"
			...
			<el-form-item class="add-item" v-else label="现场照片" prop="xianchangzhaopian">
				<img v-if="ruleForm.xianchangzhaopian.substring(0,4)=='http'" class="upload-img" v-bind:key="index" :src="ruleForm.xianchangzhaopian.split(',')[0]">
				<img v-else class="upload-img" v-bind:key="index" v-for="(item,index) in ruleForm.xianchangzhaopian.split(',')" :src="baseUrl+item">
			</el-form-item>
			...
			<el-form-item class="add-item" label="事故描述" prop="shigumiaoshu">
				<el-input
					type="textarea"
					:rows="8"
					placeholder="事故描述"
	...
	export default {
		...
		methods: {
			...
			init(type) {
				...
				if(type=='cross'){
					...
					for (var o in obj){
						...
						if(o=='xianchangzhaopian'){
							this.ruleForm.xianchangzhaopian = obj[o].split(",")[0];
				...
				this.$http.get('option/shiguleixing/shiguleixing', {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
						this.shiguleixingOptions = res.data.data;
					}
				});
			...
			// 多级联动参数
			info() {
				this.$http.get(`shigushangbao/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`shigushangbao/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
			...
			xianchangzhaopianUploadChange(fileUrls) {
				this.ruleForm.xianchangzhaopian = fileUrls.replace(new RegExp(this.$config.baseUrl,"g"),"");
			},
		}
	};
</script>
