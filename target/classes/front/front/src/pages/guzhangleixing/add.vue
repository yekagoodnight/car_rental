		...
		<el-form
			...
			<el-form-item class="add-item" label="故障类型" prop="guzhangleixing">
				<el-input v-model="ruleForm.guzhangleixing" 
					placeholder="故障类型" clearable :disabled=" false  ||ro.guzhangleixing"></el-input>
			</el-form-item>
...
<script>
	export default {
		...
		methods: {
			...
			// 多级联动参数
			info() {
				this.$http.get(`guzhangleixing/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`guzhangleixing/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
