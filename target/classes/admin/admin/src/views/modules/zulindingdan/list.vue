		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"border":"20px solid #fff","margin":"0 0px 0px","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","justifyContent":"space-between"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"padding":"10px","margin":"0px 0","flexWrap":"wrap","background":"none","display":"flex"}'>
					<el-button class="add" v-if="isAuth('zulindingdan','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia1" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"34px"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('zulindingdan','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
						<span class="icon iconfont icon-shanchu6" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"34px"}'></span>
						删除
					</el-button>


					<el-button class="btn18" v-if="isAuth('zulindingdan','支付')" :disabled="dataListSelections.length?false:true" type="success" @click="payBatch()">
						<span class="icon iconfont icon-zhifudingjin" :style='{"margin":"0 2px","fontSize":"14px","color":"#939393","height":"34px"}'></span>
						批量支付
					</el-button>
					<el-button class="btn18" v-if="isAuth('zulindingdan','租赁次数统计')" type="success" @click="chartDialog1()">
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true' prop="ispay" label="是否支付">
						<template slot-scope="scope">
							...
							<el-button v-if="scope.row.ispay!='已支付' && isAuth('zulindingdan','支付') " type="text" size="small" @click="payHandler(scope.row)">支付</el-button>
						</template>
					</el-table-column>
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('zulindingdan','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-tianjia1" :style='{"margin":"0 0px","fontSize":"12px","color":"#fff","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="btn8" v-if="isAuth('zulindingdan','归还汽车')" @click="haichejiluCrossAddOrUpdateHandler(scope.row,'cross','否','是','[1]','已归还')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 0px","fontSize":"14px","color":"#939393","height":"40px"}'></span>
								归还汽车
							</el-button>
							<el-button class="btn8" v-if="isAuth('zulindingdan','订单取消')" @click="dingdanquxiaoCrossAddOrUpdateHandler(scope.row,'cross','否','否','[1]','已取消')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 0px","fontSize":"14px","color":"#939393","height":"40px"}'></span>
								订单取消
							</el-button>
							<el-button class="btn8" v-if="isAuth('zulindingdan','故障上报')" @click="guzhangshangbaoCrossAddOrUpdateHandler(scope.row,'cross','否','是','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 0px","fontSize":"14px","color":"#939393","height":"40px"}'></span>
								故障上报
							</el-button>
							<el-button class="btn8" v-if="isAuth('zulindingdan','事故上报')" @click="shigushangbaoCrossAddOrUpdateHandler(scope.row,'cross','否','是','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 0px","fontSize":"14px","color":"#939393","height":"40px"}'></span>
								事故上报
							</el-button>
							<el-button class="btn8" v-if="isAuth('zulindingdan','车辆保险')" @click="cheliangbaoxianCrossAddOrUpdateHandler(scope.row,'cross','否','是','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 0px","fontSize":"14px","color":"#939393","height":"40px"}'></span>
								车辆保险
							</el-button>
							<el-button class="edit" v-if=" isAuth('zulindingdan','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('zulindingdan','删除') " type="primary" @click="deleteHandler(scope.row.id )">
