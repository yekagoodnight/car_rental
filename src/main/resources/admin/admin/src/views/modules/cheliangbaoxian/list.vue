		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"border":"20px solid #fff","margin":"0 0px 0px","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","justifyContent":"space-between"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"10px","alignItems":"center","flexWrap":"wrap","background":"none","display":"flex"}' >
					<div :style='{"alignItems":"center","margin":"0 10px 0 0","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">保单编号</label>
						<el-input v-model="searchForm.baodanbianhao" placeholder="保单编号" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"alignItems":"center","margin":"0 10px 0 0","display":"flex"}'>
				...
				<el-row class="actions" :style='{"padding":"10px","margin":"0px 0","flexWrap":"wrap","background":"none","display":"flex"}'>
					<el-button class="add" v-if="isAuth('cheliangbaoxian','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia1" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"34px"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('cheliangbaoxian','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
						<span class="icon iconfont icon-shanchu6" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"34px"}'></span>
						删除
					</el-button>


					<el-button class="btn18" v-if="isAuth('cheliangbaoxian','支付')" :disabled="dataListSelections.length?false:true" type="success" @click="payBatch()">
			...
			<div :style='{"border":"20px solid #fff","padding":"0","color":"#000","background":"#fff","borderWidth":"0 20px 20px","width":"100%","fontSize":"14px"}'>
				<el-table class="tables"
					...
					:border='true'
					v-if="isAuth('cheliangbaoxian','查看')"
					:data="dataList"
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true'  
						prop="baoxianmingcheng"
						label="保险名称">
						<template slot-scope="scope">
							{{scope.row.baoxianmingcheng}}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'  
						prop="baoxianleixing"
					...
					<el-table-column :resizable='true' prop="baoxianfujian" label="保险附件">
						<template slot-scope="scope">
							<el-button v-if="scope.row.baoxianfujian" type="text" size="small" @click="download(scope.row.baoxianfujian)">下载</el-button>
							<span v-else >无</span>
						</template>
					</el-table-column>
					...
					<el-table-column :resizable='true' :sortable='true' prop="ispay" label="是否支付">
						<template slot-scope="scope">
							...
							<el-button v-if="scope.row.ispay!='已支付' && isAuth('cheliangbaoxian','支付') " type="text" size="small" @click="payHandler(scope.row)">支付</el-button>
						</template>
					</el-table-column>
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('cheliangbaoxian','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-tianjia1" :style='{"margin":"0 0px","fontSize":"12px","color":"#fff","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="edit" v-if=" isAuth('cheliangbaoxian','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('cheliangbaoxian','删除') " type="primary" @click="deleteHandler(scope.row.id )">
...
<script>
	...
	export default {
		...
		methods: {
			...
			payHandler(row){
				this.$storage.set('paytable','cheliangbaoxian');
			...
			getDataList() {
				...
				let params = {
 				...
 				if(this.searchForm.baodanbianhao!='' && this.searchForm.baodanbianhao!=undefined){
					params['baodanbianhao'] = '%' + this.searchForm.baodanbianhao + '%'
				}
				...
				if(this.searchForm.baoxianleixing!='' && this.searchForm.baoxianleixing!=undefined){
					params['baoxianleixing'] = this.searchForm.baoxianleixing
				}
