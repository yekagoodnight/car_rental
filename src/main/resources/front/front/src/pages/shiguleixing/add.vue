		...
		<el-form
			...
			<el-form-item class="add-item" label="事故类型" prop="shiguleixing">
				<el-input v-model="ruleForm.shiguleixing" 
					placeholder="事故类型" clearable :disabled=" false  ||ro.shiguleixing"></el-input>
			</el-form-item>
...
<script>
	export default {
		...
		methods: {
			...
			// 多级联动参数
			info() {
				this.$http.get(`shiguleixing/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`shiguleixing/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
