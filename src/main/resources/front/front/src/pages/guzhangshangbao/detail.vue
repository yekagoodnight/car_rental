	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="''">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/guzhangshangbao?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						<div class="lable">故障时间</div>
						<div class="text "  >{{detail.guzhangshijian}}</div>
					</div>
					<div class="item">
						<div class="lable">故障地点</div>
						<div class="text "  >{{detail.guzhangdidian}}</div>
					</div>
					<div class="item">
						<div class="lable">故障类型</div>
						<div class="text "  >{{detail.guzhangleixing}}</div>
					</div>
					<div class="item">
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('guzhangshangbao','修改')&&detail.sfsh!='是'&&detail.sfsh!='否' " @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('guzhangshangbao','删除')" @click="delClick">删除</el-button>
						<el-button class="auditBtn" v-if="btnAuth('guzhangshangbao','审核') && (detail.sfsh=='待审核'||!detail.sfsh)" @click="onSH" type="warning">审核</el-button>
					</div>
				</div>
			</div>
		...
		methods: {
			...
			init() {
				...
				this.$http.get(this.tablename + '/detail/'  + this.id, {}).then(res => {
					if (res.data.code == 0) {
						...
						if(this.detail.guzhangtupian) {
							this.detailBanner = this.detail.guzhangtupian.split(",w").length>1?[this.detail.guzhangtupian]:this.detail.guzhangtupian.split(',');
						}
						this.$forceUpdate();
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/guzhangshangbao', query: params});
				}
			},
			// 下载
			download(file ){
			...
			// 修改
			editClick(){
				this.$router.push(`/index/guzhangshangbaoAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此故障上报？') .then(_ => {
					this.$http.post('guzhangshangbao/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
